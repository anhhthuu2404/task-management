...
export class TaskService {
  ...
  getList = (input: GetTaskListInputDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<TaskDto>>({
      method: 'GET',
      url: '/api/app/task',
      params: { keyword: input.keyword, filter: input.filter, categoryId: input.categoryId, departmentId: input.departmentId, assigneeId: input.assigneeId, projectId: input.projectId, priority: input.priority, status: input.status, departmentIds: input.departmentIds, onlyMyTasks: input.onlyMyTasks, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
